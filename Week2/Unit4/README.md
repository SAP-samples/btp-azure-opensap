...
In this unit you will learn how to run multiple SAP Cloud Integration tenants (SAP Integration Suite) in parallel to apply the mechanisms mentioned above. Azure Traffic Manager will play a significant role in routing the traffic intelligently to different SAP Cloud Integration tenants.
...
## Problems
> If you have any issues with the exercises, don't hesitate to open a question in the openSAP Discussion forum for this course. Provide the exact step number: "Week2Unit4, Step 1.1: Command cannot be executed. My expected result was [...], my actual result was [...]". Logs, etc. are always highly appreciated. 
...
3.15 The required certificate chain should now be visible in your folder as **0001_chain.pem**. You can open it with an editor of your choice, and check whether you can see three different certificates in the chain. 

![Certificate in text editor](./images/62.png)

> Hint: Until end of September 2021, the Let's Encrypt certificate chain was based upon the DST Root CA certificate, which was trusted by almost all clients within the public internet. Since October 2021, Let's Encrypt is using a different root certificate called ISRG Root X1. This root certificate is also trusted by a lot of clients already, but you might face certificate warnings on older devices.

3.16 Upload and activate the certificates: 

```console
cf custom-domain-upload-certificate-chain key1 0001_chain.pem
```
...
3.17 Activate the custom domain: 
...
3.18 Verify the custom domain activation: 
...
3.19 <a name="endpointmapping"></a>Finally, map the SAP Cloud Integration **runtime** endpoint to a subdomain of your domain: 
...
3.20 **Execute all of the steps for the second subaccount!**
