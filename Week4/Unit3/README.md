...
1.3 Create a new Azure Virtual Machine (Azure VM) via **Create** - **Virtual machine**.
...
> **IMPORTANT:** Make sure you note this password once set.

![Change password of new user](./images/passwd_change.png)

2.9 Now we need to create the installation directory and provide our new user with the respective rights to install software in that directory. First, change into the **/usr** directory:
...
2.12 Create another directory inside /usr/sap called **dataprovagent** running:

```shell
sudo mkdir dataprovagent
```

2.13 Change the ownership of this new directory to our dpa-user using: 
...
Since the Virtual Machine is successfully created and prepared, you can now start to install the Data Provisiong Agent on the Virtual Machine.
...
> **IMPORTANT:** Please make sure you are downloading the SDI Data Provisioning Agent for Linux, since the Virtual Machine runs on Linux! Depending on your internet connection, the download might take a few minutes (>800 MB).
...
3.6 Navigate into the extracted directory: 
...
Now that the installation has been finished, the DPAgent is ready to be configured and to be connected. In order to connect to Azure Data Explorer, the DPAgent is using the *MssqlLogReaderAdapter* - therefore, you need the Microsoft JDBC driver.
...
4.5 Move this JDBC Driver jar file into the the **lib** directory of the DPAgent:
...
<a name="hanacloudstatus"></a>Since the SAP HANA Cloud instance in SAP BTP Trial is automatically stopped after a certain time, make sure that your SAP HANA Cloud instance is up and running. This is needed since you are going to establish a connection between the SAP HANA Cloud instance and the Data Provisioning Agent in the following step. </a>
